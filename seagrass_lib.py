...
def multipart_to_singlepart(shp_fp, out_fp, output_proj = None):       
    ...
    in_spatialref = in_layer.GetSpatialRef() # Get the spatial refrence of the input shapefile
    if os.path.exists(out_fp): # Remove output file if it already exists
        os.remove(out_fp)
    dst_ds = ogr.GetDriverByName('ESRI Shapefile').CreateDataSource(out_fp) # Create output shapefile object
    if(output_proj == None): # If an output projection wasn't specified
    ...
    coordTrans = osr.CoordinateTransformation(in_spatialref, out_spatialref) # Coordinate tranformation between input shapefile and output shapefile
    feature = in_layer[0] # Get the first feature
    for id in range(feature.GetFieldCount()): # Loop through feilds
        field = feature.GetFieldDefnRef(id) # Field definition
        fname = field.GetName() # Field name
        ftype = field.GetTypeName() # Field type
        fwidth = field.GetWidth() # Field width
        if ftype == 'String': # Copy field definitions from source 
            fielddefn = ogr.FieldDefn(fname, ogr.OFTString)
            fielddefn.SetWidth(fwidth)
        else:
            fielddefn = ogr.FieldDefn(fname, ogr.OFTInteger) 
        out_layer.CreateField(fielddefn) # Create the same fields in output shapefile
    feature = in_layer.GetNextFeature() # Get the first feature
    while feature: # Loop through features
        geom = feature.GetGeometryRef() # Get geometry of feature
        geom.Transform(coordTrans) # Translate feature geomtry to the spatial reference of the output shapefile
        if geom.GetGeometryName() == 'MULTIPOLYGON': # If the input shapefile is a multipart polygon split it into a singlepart polygon
            for geom_part in geom: # For each multipart polygon that has been split into a singlepart polygon, copy all of the features over to the output shapefile
                feature_def = out_layer.GetLayerDefn() 
                polygon = ogr.CreateGeometryFromWkb(geom_part.ExportToWkb())
            ...
            for id in range(feature.GetFieldCount()):
                ...
                out_feat.SetField(id, data)
            out_layer.CreateFeature(out_feat)
        feature = in_layer.GetNextFeature() # Move on to the next feature
    out_spatialref.MorphToESRI() # Write the .prj file for output shapefile
...
def shp_to_roi(shp_fp, image_fp, output_dir, field_name = 'CLASS_NAME'):
    ...
    for unique_class in class_names: # Loop through each class
        ...
        in_layer.SetAttributeFilter(field_name + " = " + "'" + unique_class + "'") # Filter layer attributes to select features belonging to a specific class
...
def roi_classes(shp_fp, field_name = 'CLASS_NAME'):
    vec_ds = ogr.Open(shp_fp) # Open input shapefile
    in_layer = vec_ds.GetLayer() # Get layer
    if field_name == None: # If the field name containing the class information wasn't supplied as an argument, prompt the user to specify it
        field_names = [field.name for field in in_layer.schema]
...
def train_dcnn(cnnFileName, training_data_directory, class_names, numChannels, dimension, selected_sample_per_class = 20000, balanced_option = 'balanced', epochs = 500, batchSize = 256, deleted_channels = []):
    ...
    y_train = to_categorical(labels) # One hot vector conversion
    f = open(os.path.join(save_directory, 'command_window'+'.txt'), 'w') # Write model training output to .txt file
    sys.stdout = f
    history = multi_model.fit(x_train, y_train, epochs=epochs, batch_size=batchSize,validation_split=0.1, shuffle=True) # Train model with ROI data. Split the labeled ROI data into train and test datasets, shuffle the training data before each epoch 
    print(history)
    ...
    with h5py.File(cnnFileName, mode = 'w') as f: # Save trained model
        hdf5_format.save_model_to_hdf5(multi_model, f)
...
def dcnn_classification(image_fp, dcnn_fp, output_fp, bSize = 256):
    create_directory(os.path.dirname(output_fp)) # Create output directory if it doesn't already exist
    with h5py.File(dcnn_fp, mode = 'r') as f: # Read the specified dcnn model
        multi_model = hdf5_format.load_model_from_hdf5(f)
